...
mask_vector <- function(input_vector,
  ...
  if (output_warnings == TRUE & length(counts) > 1) {
    ...
    if ((exists("theoretical.lower.int") && theoretical.lower.int != effective.lower.int) | all(naively_masked_logical)) {
      ...
      warning(paste0(
        "This table may not be safe. Of all possible combinations of naively masked values, ",
...
mask_vector_wrapper <- function(tableout, threshold = 5,
  ...
  for (i in seq_along(variables_index)) {
    ...
    if (type_should_be_masked) {
      ...
      masked.counts.1 <- mask_vector(counts.1,
        threshold = threshold,
        rounding_digits = rounding_digits,
        output_warnings = output_warnings,
        total = total_count1
      )
      masked.counts.2 <- mask_vector(counts.2,
        threshold = threshold,
        rounding_digits = rounding_digits,
        output_warnings = output_warnings,
        total = total_count2
      )
      masked.pcts.1 <- mask_vector(counts.1,
        threshold = threshold,
        rounding_digits = rounding_digits,
        output_warnings = output_warnings,
        total = total_count1,
      ...
      masked.pcts.2 <- mask_vector(counts.2,
        threshold = threshold,
        output_warnings = output_warnings,
        rounding_digits = rounding_digits,
        total = total_count2,
